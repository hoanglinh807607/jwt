...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    private String getJwtFromRequest(HttpServletRequest request) {
        ...
        // Cookie được truyền theo trong requestHeader khi gửi request
        String cookie =  request.getHeader("Cookie");
        String headerAuth;
        //
        if( cookie.indexOf("Authorization") != -1){
            if(  cookie.indexOf(';', cookie.indexOf("Authorization")) != -1 )
                headerAuth = cookie.substring(cookie.indexOf("Authorization"),cookie.indexOf(';',cookie.indexOf("Authorization")));
            else  headerAuth = cookie.substring(cookie.indexOf("Authorization"));
        }else return null;
        String valueJwt = headerAuth.substring(headerAuth.indexOf('=')+1);
        // Kiểm tra xem header Authorization có chứa thông tin jwt không
        if ( StringUtils.hasText(valueJwt) && valueJwt.startsWith("Bearer ")) {
            return valueJwt.substring(7);
        }
        return null;
    }
}
